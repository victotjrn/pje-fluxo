...
begin;
CREATE OR REPLACE FUNCTION REDIST_PROC_COMP_EXCL(idProcesso integer, idOrgaoJulgadorDestino integer, idOrgaoJulgadorCargoDestino integer, isExtencao boolean, atualizarAcumulador boolean)  RETURNS integer AS $$
DECLARE
...
BEGIN
    ...
    IF $5 THEN
        -- diminui o acumulador antigo
         EXECUTE 'update client.tb_orgao_julgador_cargo c
set nr_acumulador_distribuicao = nr_acumulador_distribuicao - pt.vl_peso_distribuicao,
    nr_acumulador_processo = nr_acumulador_processo - pt.vl_peso_processual
from client.tb_processo_trf pt, client.tb_orgao_julgador oj
where oj.id_orgao_julgador = c.id_orgao_julgador
and oj.id_orgao_julgador = pt.id_orgao_julgador
and pt.id_processo_trf = '||$1||'
and c.in_recebe_distribuicao;';
        -- acrescenta o acumulador antigo
         EXECUTE 'update tb_orgao_julgador_cargo  c
set nr_acumulador_distribuicao = nr_acumulador_distribuicao + ( select pt.vl_peso_distribuicao from client.tb_processo_trf pt where pt.id_processo_trf = '||$1||') ,
    nr_acumulador_processo = nr_acumulador_processo + ( select pt.vl_peso_processual from client.tb_processo_trf pt where pt.id_processo_trf = '||$1||' )
where c.id_orgao_julgador_cargo = '||idOrgaoJulgadorCargoDestino||';';
    END IF;
